BIN = interpreter \
      ...
      jit-x64 jit-arm jit-x64-optimize
...
jit-x64-optimize: dynasm-driver.c jit-x64-optimize.h
	$(CC) $(CFLAGS) -o $@ -DJIT=\"jit-x64-optimize.h\" \
		dynasm-driver.c
jit-x64-optimize.h: jit-x64-optimize.dasc
	        $(LUA) dynasm/dynasm.lua -o $@ jit-x64-optimize.dasc
run-jit-x64-optimize: jit-x64-optimize
...
bench-jit-x64-optimize: jit-x64-optimize
	@echo
	@echo Executing Brainf*ck benchmark suite. Be patient.
	@echo
	@env PATH='.:${PATH}' BF_RUN='$<' tests/bench.py
...
clean:
	$(RM) $(BIN) \
	      ...
	      jit-x64.h jit-arm.h jit-x64-optimize.h
